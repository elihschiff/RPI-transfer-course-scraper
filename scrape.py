...
def scrape_place(type):
    ...
    states = [
        ...
        for x in states_select.find_elements_by_tag_name("option")
    ]
    states_real_names = [
        x.text for x in states_select.find_elements_by_tag_name("option")
    ]
    ...
    for i in range(len(states)):
        ...
        Select(driver.find_element_by_name(type)).select_by_value(states[i])
        # set the select box to the current state
        driver.find_element_by_css_selector(
            "[value='Get Institutions']"
        ).click()  # click button to load schools

        # get list of schools for this state
        schools_select = driver.find_element_by_name("sbgi_code")
        schools = [
            x.get_attribute("value")
            for x in schools_select.find_elements_by_tag_name("option")
        ]
...
def scrape_school(schools, place, type, real_name):
    for school in schools:
        ...
        Select(driver.find_element_by_name(type)).select_by_value(place)
        # repick the state
